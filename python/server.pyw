...
class myHandler(BaseHTTPRequestHandler):
    ...
    def get_module(self):
        ...
        index = DEFAULTHEADER
        modulesDir = path.join(os.getcwd(), 'modules')
    ...
    def do_GET(self):
        ...
        try:
            ...
            self.end_headers()
            plainTypes = ["png","jpg","gif","javascript","css"]
            if any(x in mimetype for x in plainTypes):
...
'''Bulid the basic folders and default values'''
modulesDir = path.join(os.getcwd(), 'modules')
if not os.path.exists(modulesDir):
    os.makedirs(modulesDir)
for fname in listdir(modulesDir):
    fpath = os.path.join(modulesDir, fname)
    if "_" not in fpath and "README.md" not in fpath:
